...
(function () {
    var anonname, builtin, funlab, funstack, functions, lex, lookahead, member, prevtoken, stack, syntax = {}, token, verb, xmode, xtype, tx = /^([(){}[.,:;'"~]|\](\]>)?|\?>?|==?=?|\/(\*(global)*|=|)|\*[\/=]?|\+[+=]?|-[-=]?|%[=>]?|&[&=]?|\|[|=]?|>>?>?=?|<([\/=%\?]|\!(\[|--)?|<=?)?|\^=?|\!=?=?|[a-zA-Z_$][a-zA-Z0-9_$]*|[0-9]+([xX][0-9a-fA-F]+|\.[0-9]*)?([eE][+-]?[0-9]+)?)/, sx = /^((\\[^\x00-\x1f]|[^\x00-\x1f'\\])*)'/, qx = /^((\\[^\x00-\x1f]|[^\x00-\x1f"\\])*)"/, rx = /^(\\[^\x00-\x1f]|\[(\\[^\x00-\x1f]|[^\x00-\x1f\\\/])*\]|[^\x00-\x1f\\\/\[])+\/[gim]*/, lx = /\*\/|\/\*/, gx = /^([a-zA-Z_$][a-zA-Z0-9_$]*)/, hx = /^[\x00-\x20,]*(\*\/)?/, wx = /^\s*(\/\/.*\r*$)?/;
    jslint = function (s) {
        functions = [];
        error.free = true;
        xmode = false;
    ...
    function Lex(lines, filename) {
        ...
        jslint.lines = lines;
        var line = 0, character = 0, from = 0, s = lines[0];
        ...
        this.token = function () {
            function string(x) {
                ...
                if (r) {
                    ...
                    if (xmode == 'script') {
                        if (t.indexOf('<\/') >= 0) {
                            throw error('Expected "...<\\/..." and instead saw "...<\/...".', token);
                        }
                    }
                    return it('(string)', r[1]);
                } else {
                    if (!xmode) {
                        throw error("Unclosed string: " + s, line, character);
                    }
                    while (error.free) {
                        if (!nextLine()) {
            ...
            if (r) {
                ...
                if (c.isDigit()) {
                    if (token.id === '.') {
                        throw error("A decimal fraction should have a zero before the decimal point.", token);
                    }
                    if (!isFinite(Number(t))) {
                        throw error("Bad number: '" + t + "'.", line, character);
                    }
                    ...
                    if (t.substr(t.length - 1) === '.') {
                        throw error("A trailing decimal point can be confused with a dot: '" + t + "'.", line, character);
                    }
                    return it('(number)', t);
                }
                if (t === '"') {
                    return (xmode === '"' || xmode === 'string') ? it('(punctuator)', t) : string(qx);
                }
                if (t === "'") {
                    return (xmode === "'" || xmode === 'string') ? it('(punctuator)', t) : string(sx);
                }
                if (t === '/*') {
                    while (error.free) {
                        i = s.search(lx);
            ...
            throw error("Unexpected token: " + (t || s.substr(0, 1)), line, character);
        };
        ...
        if (t) {
            ...
            if (typeof funlab[t] === 'string') {
                switch (funlab[t]) {
                    ...
                    case'global':
                        if (type === 'var') {
                            throw error('Var ' + t + ' was used before it was declared.', prevtoken);
                        }
    ...
    function advance(id, t) {
        ...
        switch (prevtoken.id) {
            case'(number)':
                if (token.id === '.') {
                    throw error("A dot following a number can be confused with a decimal point.", prevtoken);
                }
                break;
            case'-':
            ...
            if (t) {
                if (token.id === '(end)') {
                    ...
                    throw error("Expected '" + id + "' to match '" +
                        t.id + "' from line " + (t.line + 1) + " and instead saw '" + token.value + "'.");
                }
            } else {
        ...
        if (l && token.id != '{' && token.id != '}' && token.id != ']') {
            throw error("Strict line ending error: '" +
    ...
    function parse(rbp, initial) {
        var l, left, o;
        if (token.id && token.id === '/') {
            if (prevtoken.id != '(' && prevtoken.id != '=' && prevtoken.id != ':' && prevtoken.id != ',' && prevtoken.id != '=') {
                throw error("Expected to see a '(' or '=' or ':' or ',' preceding a regular expression literal, and instead saw '" +
                    prevtoken.value + "'.", prevtoken);
            }
            advanceregex();
        }
        ...
        if (initial && prevtoken.fud) {
            ...
            if (prevtoken.nud) {
                ...
                if (token.type === '(number)' && prevtoken.id === '.') {
                    throw error("A leading decimal point can be confused with a dot: ." + token.value, prevtoken);
                }
            ...
            if (initial && !o) {
                throw error("Expected an assignment or function call and instead saw an expression.", prevtoken);
            }
        }
        if (l) {
            funlab[l] = 'label';
            ...
            if (left) {
                ...
                if (left == syntax['function']) {
                    if (jslint.jscript) {
                        ...
                        throw error("Expected an identifier in an assignment, and instead saw a function invocation.", prevtoken);
                    }
                }
            }
    ...
    function suffix(s, f) {
        ...
        x.led = function (left) {
            if (jslint.plusplus) {
                throw error("This operator is considered harmful: " + this.id, this);
            }
            return [f, left];
        };
        return x;
    ...
    function reachable(s) {
        ...
        while (error.free) {
            ...
            if (t.id != '(endline)') {
                if (t.id === 'function') {
                    throw error("Inner functions should be listed at the top of the outer function.", t);
                }
    ...
    function statement() {
        ...
        if (t.identifier && !t.reserved && peek().id === ':') {
            ...
            if (!token.labelled) {
                throw error("Label '" + t.value + "' on unlabelable statement '" + token.value + "'.", token);
            }
            if (t.value.toLowerCase() == 'javascript') {
                throw error("Label '" + t.value + "' looks like a javascript url.", token);
            }
            token.label = t.value;
            t = token;
        }
        ...
        if (!t.block && error.free) {
            if (token.id != ';') {
                throw error("Expected ';' and instead saw '" + token.value + "'.", prevtoken.line, prevtoken.from + prevtoken.value.length);
            }
            advance(';');
        }
    }
    ...
    type('(string)', idValue);
    syntax['(identifier)'] = {
        type: '(identifier)', lbp: 0, identifier: true, nud: function () {
    ...
    assignop('/=', 'assigndiv', 20).nud = function () {
        throw error("A regular expression literal can be confused with '/='.");
    };
    ...
    infix('==', function (left) {
        var t = token;
        if ((t.type === '(number)' && !+t.value) || (t.type === '(string)' && !t.value) || t.type === 'true' || t.type === 'false' || t.type === 'undefined' || t.type === 'null') {
    ...
    infix('(', function (left) {
        ...
        advance(')');
        if (left && left.value == 'parseInt' && n == 1) {
    ...
    (function (x) {
        ...
        x.fud = function () {
            throw error("Expected to see a statement and instead saw a block.");
        };
    })(delim('{'));
                ...
                case'case':
                    switch (verb) {
                        ...
                        case'throw':
                            break;
                        default:
                            throw error("Expected a 'break' statement before 'case'.", prevtoken);
                    }
                    advance('case');
                    parse(20);
                    g = true;
                ...
                case'default':
                    switch (verb) {
                        ...
                        case'throw':
                            break;
                        default:
                            throw error("Expected a 'break' statement before 'default'.", prevtoken);
                    }
                    advance('default');
                    g = true;
                    advance(':');
            ...
            for (k in f) {
                if (f[k] === 'global') {
                    ...
                    while (error.free) {
                        cc = c['(context)'];
                        if (!cc) {
                            if ((!funlab[k] || funlab[k] === 'var?') && builtin[k] != 9) {
                                funlab[k] = 'var?';
                                f[k] = 'global?';
                            }
                            break;
        ...
        for (i = 0; i < functions.length; i += 1) {
            ...
            for (k in f) {
                if (k.charAt(0) != '(') {
                    switch (f[k]) {
                        case'var':
                            s.push('<tt>' + k + '</tt><small>&nbsp;(unused)</small>');
                            break;
                        case'var*':
                            s.push('<tt>' + k + '</tt>');
                            break;
                        case'var!':
                            ...
                            break;
                        case'var.':
                            s.push('<tt>' + k + '</tt><small>&nbsp;(outer)</small>');
                            break;
                    }
                }
            }
        ...
        HTML: {
            ...
            doTagName: function (n, p) {
                ...
                if (x) {
                    ...
                    do {
                        if (i <= 0) {
                            throw error('A <' + n + '> must be within the body', prevtoken);
                        }
                        i -= 1;
                    } while (stack[i] !== 'body');
                }
            ...
            tag: {
                ...
                q: {},
                samp: {},
                script: {parent: ' head body p div span abbr acronym address bdo blockquote cite code del dfn em ins kbd pre samp strong th td var '},
                select: {},
                small: {},
                span: {},
                strong: {},
    ...
    function xmlword(tag) {
        ...
        if (!token.identifier) {
            if (token.id === '<') {
                throw error(tag ? "Expected &lt; and saw '<'" : "Missing '>'", prevtoken);
            } else {
    ...
    function xml() {
        ...
        while (error.free) {
            switch (token.value) {
                case'<':
                    ...
                    if (!xtype) {
                        ...
                        e = xmltype[xtype].doTagName(n, stack[stack.length - 1]);
                    }
                    while (error.free) {
                        if (token.id === '/') {
                            advance('/');
                        ...
                        switch (xmltype[xtype].doAttribute(n, a)) {
                            case'script':
                                ...
                                statements();
                                if (token.id !== q) {
                                    throw error('Missing close quote on script attribute');
                                }
                                xmode = 'xml';
                                advance(q);
                                break;
                            case'value':
                                advance('=');
                                if (!token.identifier && token.type != '(string)' && token.type != '(number)') {
                ...
                case'</':
                    ...
                    if (t != n) {
                        throw error('Expected </' + t + '> and instead saw </' +
                            n + '>');
                    }
                    if (token.id !== '>') {
                        throw error("Expected '>'");
                ...
                case'<?':
                    while (error.free) {
                        ...
                        if (token.id === '<?' || token.id === '<' || token.id === '>' || token.id === '(end)') {
                            throw error("Missing '?>'.", prevtoken);
                        }
                    }
                    lex.skip('?>');
            ...
            if (!lex.skip('')) {
                if (stack.length) {
                    t = stack.pop();
                    throw error('Unmatched tag <' + t + '>', prevtoken);
                }
                return;
            }
            advance();
...
jslint.debug = false;
if (!jslint(WScript.StdIn.ReadAll())) {
    var e = jslint.error;
